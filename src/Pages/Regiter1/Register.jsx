...

      {
        data?.map((item , index)=>(
          ...
          <h3>{item.title}</h3>
           ...
           {
            videos.includes(item.id) ?      <img onClick={()=>handleVideo2(item.id)} width={942} height={494} src={`http://64.226.102.92:8001/camera/video_feed/${item.id}/`} alt=""  /> :      <img onClick={()=>handleVideo(item.id)} width={942} height={494} src={Video} alt=""  /> 
           }
         </div>
         </div>
        ))
